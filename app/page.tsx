...
export default function Home() {
  ...
  async function createGame() {
    ...
    const { data, error: err } = await supabase
      .from('games')
      .insert({ code, total_rounds: totalRounds })
      .select()
      .single()
      ...
      <div className="flex-1 flex flex-col px-5 py-8 max-w-xl mx-auto w-full relative z-10">
        ...
        <div className="card p-5 mb-4">
          <div className="flex items-center gap-3 mb-4 pb-3 border-b" style={{ borderColor: 'var(--border)' }}>
            ...
            <span className="label-micro flex-1">Number of Rounds</span>
            <span className="font-display text-2xl tabular" style={{ color: 'var(--mint)' }}>{totalRounds}</span>
          </div>
          <div className="flex gap-2">
            {[1, 2, 3, 4, 5].map(n => (
              <button key={n} onClick={() => setTotalRounds(n)}
                className="flex-1 py-3 rounded-xl font-display text-xl transition-all"
                style={{
                  background: totalRounds === n
                    ? 'linear-gradient(135deg, rgba(0,255,135,0.2) 0%, rgba(0,255,135,0.08) 100%)'
                    : 'var(--surface-2)',
                  border: `1.5px solid ${totalRounds === n ? 'var(--mint)' : 'var(--border)'}`,
                  color: totalRounds === n ? 'var(--mint)' : 'var(--text-muted)',
                  boxShadow: totalRounds === n ? '0 0 12px rgba(0,255,135,0.15)' : 'none',
                }}>
                {n}
              </button>
            ))}
          ...
          <p className="text-xs mt-3" style={{ color: 'var(--text-muted)' }}>
            {totalRounds === 1 ? 'One question, 3 minutes — classic match.' : `${totalRounds} questions back-to-back. Overall winner takes the bragging rights.`}
          </p>
        </div>

        {/* CTA */}
        <div className="mb-6">
          <button onClick={createGame} disabled={loading} className="btn-primary w-full py-5 text-xl">
            {loading ? 'SETTING UP...' : `CREATE ${totalRounds > 1 ? `${totalRounds}-ROUND ` : ''}MATCH`}
          </button>
        ...
        <div className="card p-5 mb-4">
          ...
          <div className="space-y-3">
            {[
              ['01', 'Create a match', 'Pick 1–5 rounds, host gets a room code'],
              ['02', 'Squad joins', 'The Shots open the link on their phones'],
              ['03', 'Pick your questions', 'Premier League, World Cup, Ballon d\'Or…'],
