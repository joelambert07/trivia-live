...
// ── AnswerEditor lives OUTSIDE the page component so React never
//    unmounts it on re-render, keeping mobile keyboard focus. ──────────────
interface AnswerEditorProps {
  q: EditableQuestion
  msgId: string
  msg: { id: string; text: string; ok: boolean } | null
  onFieldChange: (field: 'category' | 'question', value: string) => void
  onAnswerChange: (idx: number, field: 'match' | 'display', value: string) => void
  onAddAnswer: () => void
  onRemoveAnswer: (idx: number) => void
  onMoveAnswer: (idx: number, dir: -1 | 1) => void
}

function AnswerEditor({
  q, msgId, msg,
  onFieldChange, onAnswerChange, onAddAnswer, onRemoveAnswer, onMoveAnswer,
}: AnswerEditorProps) {
  return (
      ...
      <div className="space-y-1.5">
        ...
        {q.answers.map((a, i) => (
          <div key={i} className="flex items-center gap-2">
            ...
            <input value={a.match}
              onChange={e => onAnswerChange(i, 'match', e.target.value)}
            ...
            <input value={a.display}
              onChange={e => onAnswerChange(i, 'display', e.target.value)}
              placeholder="e.g. Wayne Rooney (2004)"
              className="flex-1 px-2.5 py-1.5 rounded-lg text-xs outline-none"
              style={{ background: 'var(--surface-3)', border: '1px solid var(--border)', color: 'var(--text)' }} />
            <div className="flex gap-0.5 shrink-0">
              <button onClick={() => onMoveAnswer(i, -1)} disabled={i === 0}
              ...
              <button onClick={() => onMoveAnswer(i, 1)} disabled={i === q.answers.length - 1}
                className="w-6 h-6 rounded text-xs flex items-center justify-center disabled:opacity-30"
                style={{ background: 'var(--surface-2)' }}>↓</button>
              <button onClick={() => onRemoveAnswer(i)}
...
export default function AdminQuestionsPage() {
  ...
  function updateField(id: string, field: 'category' | 'question', value: string) {
    setQs(prev => prev.map(q => q.id === id ? { ...q, [field]: value, dirty: true } : q))
  }
  ...
  function newFieldChange(field: 'category' | 'question', value: string) {
    setNewQ(prev => prev ? { ...prev, [field]: value } : null)
  }

  function newAnswerChange(idx: number, field: 'match' | 'display', value: string) {
    setNewQ(prev => {
      if (!prev) return null
  ...
  function newAddAnswer() {
    setNewQ(prev => prev ? { ...prev, answers: [...prev.answers, { match: '', display: '' }] } : null)
  }

  function newRemoveAnswer(idx: number) {
    setNewQ(prev => prev ? { ...prev, answers: prev.answers.filter((_, i) => i !== idx) } : null)
  }

  function newMoveAnswer(idx: number, dir: -1 | 1) {
    setNewQ(prev => {
      if (!prev) return null
      const arr = [...prev.answers]
      const target = idx + dir
      if (target < 0 || target >= arr.length) return prev;
      [arr[idx], arr[target]] = [arr[target], arr[idx]]
      return { ...prev, answers: arr }
    })
  }
    ...
    <div className="min-h-screen stadium-bg noise">
      <div className="max-w-3xl mx-auto px-5 py-8 relative z-10">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <button onClick={startNewQ} className="btn-primary px-4 py-2.5 text-sm font-bold">
            + NEW QUESTION
          </button>
        </div>
          ...
          <div className="card p-5 mb-5 relative overflow-hidden">
            ...
            <div className="pl-3">
              ...
              <AnswerEditor
                ...
                msgId="new"
                msg={msg}
                onFieldChange={newFieldChange}
                onAnswerChange={newAnswerChange}
                onAddAnswer={newAddAnswer}
                onRemoveAnswer={newRemoveAnswer}
                onMoveAnswer={newMoveAnswer}
              />
              {msg?.id === 'new' && (
            ...
            <div className="space-y-2">
              {catQs.map(q => (
                <div key={q.id} className="card overflow-hidden">
                  <button
                    onClick={() => setQs(prev => prev.map(p => p.id === q.id ? { ...p, expanded: !p.expanded } : p))}
                    ...
                    <div className="px-4 pb-4 border-t" style={{ borderColor: 'var(--border)' }}>
                      <div className="pt-4">
                        <AnswerEditor
                          ...
                          msgId={q.id}
                          msg={msg}
                          onFieldChange={(field, value) => updateField(q.id, field, value)}
                          onAnswerChange={(idx, field, value) => updateAnswer(q.id, idx, field, value)}
                          onAddAnswer={() => addAnswer(q.id)}
                          onRemoveAnswer={idx => removeAnswer(q.id, idx)}
                          onMoveAnswer={(idx, dir) => moveAnswer(q.id, idx, dir)}
                        />
                        {msg?.id === q.id && (
                          <p className="text-xs font-semibold mt-2" style={{ color: msg.ok ? 'var(--mint)' : 'var(--red)' }}>{msg.text}</p>
                        )}
